/*
Es07a: shadow rendering with shadow mapping technique - PART 2
...
- conclusion of Es06c, with object shaders now using the shadow map computed in the first rendering step
...
// variables and parameters for the directional light "view frustum"
...
int main_old(){
    ...
    Shader illumination_shader = Shader("shaders/21_ggx_tex_shadow.vert", "shaders/22_ggx_tex_shadow.frag");
...
void RenderObjects(Shader &shader, Model &planeModel, Model &cubeModel, Model &sphereModel, Model &bunnyModel, GLint render_pass, GLuint depthMap)
{
    ...
    if (render_pass==RENDER)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, depthMap);
        shadowLocation = glGetUniformLocation(shader.Program, "shadowMap");
...
void PrintCurrentShader(int subroutine)
{
    std::cout << "Current shader subroutine: " << subroutine+1 << " - " << shaders[subroutine]  << std::endl;
}
